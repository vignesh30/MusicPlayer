...
public class PlayList extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSongListTemp.clear();
        mSongList.clear();
        adapter = new ListAdapter(PlayList.this, R.layout.item_list_image, mSongListTemp);
        mListView.setAdapter(adapter);
        progress.setVisibility(View.GONE);
        parentLayout = findViewById(android.R.id.content);
        //load songs from sqlLite db
        helper  = new SqlLiteDB(getApplicationContext());
        Cursor c = helper.getPlayLists();
        if (c.getCount() == 0) {
           Snackbar.make(parentLayout, "No song in your Play List", Snackbar.LENGTH_INDEFINITE).show();
        } else {
    ...
    private void init(ArrayList<Values> songList) {
        ...
        for (int j = 0; j < size; j++) {
            ...
            final int k;
            k = j;
            final Button btnPage = new Button(PlayList.this);
            LayoutParams lp = new LayoutParams(75, 75);
